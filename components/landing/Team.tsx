...
export function Team() {
  return (
    <section
      id="team"
      className="relative overflow-hidden px-6 lg:px-10 py-24 bg-cream border-t border-border"
    >
      <DecorativeLayer>
        <ScrollFloat
          className="absolute -top-6 left-[6%] h-32 w-32 text-electric/35"
          yRange={[-18, 30]}
        >
          <DoodleCircle className="h-full w-full" />
        </ScrollFloat>
        <ScrollFloat
          className="absolute top-20 right-[10%] h-28 w-28 text-hot-pink/40"
          yRange={[18, -28]}
        >
          <DoodleCircle className="h-full w-full" tone="pink" />
        </ScrollFloat>
        <MouseParallax
          className="absolute left-[8%] top-44 h-20 w-20 opacity-70"
          strength={10}
        >
          <Sparkle className="h-full w-full" />
        </MouseParallax>
        <ScrollFloat
          className="absolute right-[8%] bottom-12 h-36 w-36 opacity-60"
          yRange={[-12, 22]}
        >
          <PaperAirplane className="h-full w-full" />
        </ScrollFloat>
        <Drift
          className="absolute left-1/2 top-10 h-12 w-40 -translate-x-1/2 opacity-50"
          delay={0.4}
        >
      ...
      <div className="relative max-w-[85vw] mx-auto">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-ink/70">
            The ClickMasters core team blends product strategy, brand-sensitive
            design, and senior mobile execution so every milestone feels
            lighter, faster, and sharper.
          </p>
        </div>
        ...
        <div className="relative mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-[repeat(4,minmax(0,1fr))_1.1fr]">
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-6">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span
                    className={`${member.accent} mt-1 inline-flex h-10 w-10 items-center justify-center rounded-full text-cream shadow-sm`}
                  >
                ...
                <div className="mt-5 flex items-center gap-2 text-ink/70">
                  <a
                    ...
                    className="rounded-full border border-ink/10 p-2 transition hover:border-hot-pink hover:text-hot-pink"
                  >
                  ...
                  <a
                    ...
                    className="rounded-full border border-ink/10 p-2 transition hover:border-electric hover:text-electric"
                  >
                ...
                <a
                  href="#contact"
                  className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-semibold text-ink story-link"
                >
          ...
          >
            <DecorativeLayer>
              <Drift
                className="absolute -right-6 top-4 h-28 w-28 opacity-60"
                delay={0.2}
              >
                <DoodleCircle className="h-full w-full" tone="ink" />
              </Drift>
              <ScrollFloat
                className="absolute left-4 top-28 h-12 w-28 opacity-65"
                yRange={[-10, 18]}
              >
                <DottedTrail className="h-full w-full" tone="ink" />
              </ScrollFloat>
              <ScrollFloat
                className="absolute bottom-8 right-6 h-20 w-20 opacity-80"
                yRange={[12, -10]}
              >
            ...
            <div className="relative">
              ...
              <h3 className="mt-6 font-display text-4xl font-extrabold leading-none">
                Hire us
              </h3>
              <p className="mt-4 max-w-xs text-base leading-7 text-ink/75">
                Let&apos;s build something exceptional together with a team that
                moves like product partners, not vendors.
              </p>
            </div>
            ...
            <div className="relative space-y-6">
              <div className="rounded-[24px] border border-ink/12 bg-white/75 p-5 backdrop-blur-sm">
                <p className="text-sm leading-6 text-ink/75">
                  “The best team we&apos;ve worked with — fast, collaborative,
